...
class Cart extends Component {
  ...
  handleQuantityChange = (productId, change) => {
    // Call a function passed via props to handle quantity changes
    this.props.updateQuantity(productId, change);
  };

  calculateTotalPrice = () => {
    return this.props.cartItems.reduce((total, item) => {
      const itemPrice = item.prod_prices.find(price => price.currency.symbol === this.getCurrencySymbol()); // Adjust as per desired currency symbol
      if (itemPrice) {
  ...
  getCurrencySymbol = () => {
    // Assuming all products have prices in the same currency, we use the first product's currency symbol.
    ...
    return "$"; // Default to "$" if no currency symbol is found
  };
  ...
  render() {
    const { cartItems, visibility } = this.props;
      ...
      <div className="cart-modal" style={{ display: visibility ? "block" : "none" }}>
        <div className="cart">
                ...
                <div className="cart-product-detail">
                  ...
                  <div className="cart-product-price">
                    {product.prod_prices.map((price, idx) => (
                      <p className="product-detail-price" key={idx}>
          ...
          <div className="cart-price">
            <p className="cart-total">Total</p><p className="cart-total-price">{this.getCurrencySymbol()}{this.calculateTotalPrice()}</p>
          </div>
